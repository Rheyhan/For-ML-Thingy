...
if __name__ == "__main__":
    ...
    simple_network = NeuralNetwork(no_of_in_nodes=2, no_of_out_nodes=3, 
                                   no_of_hidden_nodes=5, learning_rate=0.3, bias=1)
    ...
    #showing evaluate correct and wrong with plot
    for i in range(len(train_data)):
        z=(simple_network.run(train_data[i])).argmax()
        if train_labels[i]==z:
            plt.scatter(train_data[i][0], train_data[i][1], c=colours[z], s=20)
        else:
            plt.scatter(train_data[i][0], train_data[i][1], c="black", s=20)
    plt.show()